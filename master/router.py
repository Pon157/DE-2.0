...
from utils.emoji import em, styled_button, strip_icon
...
from config import (SUPER_ADMIN_ID, MASTER_BOT_TOKEN, AD_MAX_LEN,
                    AD_BROADCAST_COOLDOWN_DAYS, FLOW_MINIAPP_URL)
...
def kb(rows):
    def _btn(item):
        if len(item) == 3:
            ...
            if kind == "web_app":
                return InlineKeyboardButton(text=text, web_app=WebAppInfo(url=value))
...
CYCLES = {
    ...
    "cyc_banconfirm": ("ban_confirm_enabled", [False, True]),
    "cyc_infoaccess": ("info_access", ["admins", "all"]),
    "cyc_anonsugg": ("anon_suggestion_enabled", [False, True]),
}
...
async def surveyfinish_save(m: Message, state: FSMContext):
    ...
    finish_text = (m.html_text or "").strip()
...
@router.callback_query(F.data.startswith("suggapprovedtext:"))
async def suggapprovedtext_start(c: CallbackQuery, state: FSMContext):
    bot_id = int(c.data.split(":")[1])
    cb, is_owner = await _access(bot_id, c.from_user.id)
    if not cb or not is_owner:
        await c.answer("Только владелец", show_alert=True); return
    cur = getattr(cb, "suggestion_approved_text", "🎉 Ваш пост опубликован!")
    await state.update_data(bot_id=bot_id)
    await state.set_state(St.sugg_approved_text)
    await c.message.answer(
        f"Текущий текст при одобрении предложки:\n<i>{cur}</i>\n\nПришлите новый текст (HTML). Отмена: /cancel"
    )
    await c.answer()


@router.message(St.sugg_approved_text)
async def suggapprovedtext_save(m: Message, state: FSMContext):
    data = await state.get_data()
    async with Session() as s:
        obj = await s.get(ChildBot, data["bot_id"])
...
async def suggrejectedtext_start(c: CallbackQuery, state: FSMContext):
    bot_id = int(c.data.split(":")[1])
    cb, is_owner = await _access(bot_id, c.from_user.id)
    if not cb or not is_owner:
        await c.answer("Только владелец", show_alert=True); return
    cur = getattr(cb, "suggestion_rejected_text", "❌ Ваш пост отклонён.")
    await state.update_data(bot_id=bot_id)
    await state.set_state(St.sugg_rejected_text)
    await c.message.answer(
        f"Текущий текст при отклонении предложки:\n<i>{cur}</i>\n\nПришлите новый текст (HTML). Отмена: /cancel"
    )
    await c.answer()


@router.message(St.sugg_rejected_text)
async def suggrejectedtext_save(m: Message, state: FSMContext):
    data = await state.get_data()
    async with Session() as s:
        obj = await s.get(ChildBot, data["bot_id"])
...
async def closebtn_edit(c: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(bot_id=bot_id, last_msg_id=c.message.message_id)
    cur = cb.close_ticket_button_text or "❌ Закрыть обращение"
...
async def richwelcome_toggle(c: CallbackQuery):
    bot_id = int(c.data.split(":")[1])
    cb, is_owner = await _access(bot_id, c.from_user.id)
    if not cb or not is_owner:
        await c.answer("Только владелец", show_alert=True); return
    ...
    async with Session() as s:
        obj = await s.get(ChildBot, bot_id)
        obj.rich_welcome = not obj.rich_welcome
        new_val = obj.rich_welcome
        await s.commit()
    await c.answer(f"{em('check')} Рич-текст: {'вкл' if new_val else 'выкл'}")
    await bot_menu(c.model_copy(update={"data": f"bot:{bot_id}"}))
...
async def btnadd(c: CallbackQuery, state: FSMContext):
    ...
    if cb.bot_type != BotType.survey:
        ...
        rows.append([("✉️➕ Инлайн: открыть тему", "bk:inline_ticket"),
