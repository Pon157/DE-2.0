    ...
    for mid in result.single_ids:
        ...
        if forward:
            if has_buttons and markup:
                await bot.copy_message(target_chat_id, source_chat_id, mid, reply_markup=markup)
            else:
...
async def _publish_fallback_copy(bot: Bot, cfg: ChildBot, *, text: str,
    ...
    if origin_message_ids:
        ...
        sent_ids = [m.message_id for m in await bot.copy_messages(cfg.channel_id, origin_chat_id, ids, message_thread_id=thread_id)]
        ...
        if markup:
            expl = await bot.send_message(cfg.channel_id, "к сожалению, телеграм мне не позволяет прикреплять кнопки к альбомам. Но я могу отправить кнопки следующим сообщением.", message_thread_id=thread_id)
            single_ids.append(expl.message_id)
            btn_msg = await bot.send_message(cfg.channel_id, "👆 Кнопки к посту выше", message_thread_id=thread_id, reply_markup=markup)
...
async def _publish_reconstructed(bot: Bot, cfg: ChildBot, *, text: str,
    ...
    if cfg.channel_delivery_mode == "copy" and origin_chat_id \
        ...
        if origin_message_ids:
            ...
            if markup:
                expl = await bot.send_message(cfg.channel_id, "к сожалению, телеграм мне не позволяет прикреплять кнопки к альбомам. Но я могу отправить кнопки следующим сообщением.", message_thread_id=thread_id)
                btn_msg = await bot.send_message(cfg.channel_id, "👆 Кнопки к посту выше", message_thread_id=thread_id, reply_markup=markup)
                return PublishResult(sent_ids, [expl.message_id, btn_msg.message_id], btn_msg.message_id)
    ...
    if media_group:
        ...
        sent_album = list(await bot.send_media_group(cfg.channel_id, media_objs, message_thread_id=thread_id))
    ...
    async def decide(c: CallbackQuery, bot: Bot, bot_db_id: int):
        ...
        if approve:
            try:
                ...
                post_kwargs = dict(html_text=sg.html_text, file_id=sg.media_file_id,
                                  media_type=sg.media_type, media_group=group,
                                  origin_chat_id=sg.origin_chat_id, origin_message_id=sg.origin_message_id,
                                  origin_message_ids=sg.origin_message_ids,
                                  use_template=(cfg.channel_delivery_mode != "copy"),
                                  buttons_json=None,
                                  buttons_mode="template")
                await publish(bot, cfg, **post_kwargs)
                ...
                if c.message.text:
                    ...
                    await c.message.edit_caption(caption=text, parse_mode="HTML", reply_markup=None)

                try:
                    await bot.send_message(sg.user_id, f"{em('check')} Ваша предложенная запись опубликована!")
                except Exception:
                    pass
            except Exception as e:
                await c.message.answer(f"Ошибка публикации: {e}")
                async with Session() as s:
                    await s.execute(sa_update(Suggestion).where(Suggestion.id == sg_id).values(status="pending"))
                    await s.commit()
                return
        else:
            text = c.message.html_text or c.message.caption or ""
            ...
            if c.message.text:
                ...
                await c.message.edit_caption(caption=text, parse_mode="HTML", reply_markup=None)
            try:
                await bot.send_message(sg.user_id, f"{em('cross')} Ваша предложенная запись была отклонена.")
            except Exception:
                pass

        await c.answer("Обработано")
