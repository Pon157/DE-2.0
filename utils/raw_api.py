...
async def get_star_balance(bot: Bot) -> int | None:
    """Баланс Stars бота — getMyStarBalance (Bot API 9.6, апрель 2026).
    aiogram>=3.28 уже даёт типизированный bot.get_my_star_balance(), поэтому
    сырой вызов больше не нужен — оставлен только защитный try/except."""
    try:
