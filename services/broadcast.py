...
async def run_broadcast(token: str, bot_id: int, *, target: str,
    ...
    rich — НОВОЕ: рассылка чисто текстовых сообщений (без медиа) через
    send_rich_message вместо обычного send_message — расширение рич-текста
    "на все экраны" по запросу, Pro-функция (гейт уже сделан вызывающим
    кодом в master/router.py перед вызовом run_broadcast, здесь просто
    исполняем). Комбинация рич-сообщения с медиа Bot API не поддерживает —
    в этом случае rich игнорируется независимо от значения флага."""
...
async def _send(bot: Bot, chat_id: int, text, media_bytes: bytes | None,
                ...
                reusable_file_id: str | None, rich: bool = False) -> str | None:
    ...
    if rich and text:
        try:
            await bot.send_rich_message(chat_id, InputRichMessage(html=text))
